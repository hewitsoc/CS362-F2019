...
int minionRefactor(int choice1, int choice2, int currentPlayer, struct gameState *state, int handPos)
{
    int i;
    int j;
    //+1 action
...
int ambassadorRefactor(int choice1, int choice2, int currentPlayer, struct gameState *state, int handPos)
{
...
int mineRefactor(int choice1, int choice2, int currentPlayer, struct gameState *state, int handPos)
{
    int i;
    int k;
    ...
    for (i = 0; i < state->handCount[currentPlayer]; i++)
    {
        if (state->hand[currentPlayer][i] == k)
        {
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
    ...
    case mine:
        ...
        if (mineRefactor(choice1, choice2, currentPlayer, state, handPos) < 0 )
        {
            return -1;
        }
        else
    ...
    case baron:
        baronRefactor(choice1, currentPlayer, state);
        return 0;
    ...
    case minion:
        minionRefactor(choice1, choice2, currentPlayer, state, handPos);
        return 0;
    ...
    case tribute:
        tributeRefactor(currentPlayer, nextPlayer, tributeRevealedCards, state);
        return 0;

    case ambassador:
        
        if (ambassadorRefactor(choice1, choice2, currentPlayer, state, handPos) < 0 )
        {
            return -1;
        }
        else
